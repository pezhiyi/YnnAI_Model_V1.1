  ...
  return [
    {
      ...
      supportedControlNets: [67, 133]  // 支持的ControlNet预处理器ID
    }
  ];
}
...
export function getModelIdForOption(processingOptionId: string): string {
  ...
  return modelMapping[processingOptionId] || modelMapping['anime']; // 默认返回 Leonardo Anime XL 模型
}
...
export async function processImageWithAdvancedFeatures(
  ...
  try {
    ...
    // 获取Cute Emotes元素
    const cuteEmotesElement = getElementById("01b6184e-3905-4dc7-9ec6-4f09982536d5");
...
export async function generateImageWithPythonMethod(
  ...
  try {
    ...
    const genPayload = {
      ...
      modelId: options.modelId || 'aa77f04e-3eec-4034-9c07-d0f619684628', // Leonardo Kino XL
  ...
  return [
    {
      ...
      initStrength: 0.85, // Image to Image参数：85
      width: 1024,
      height: 1024,
      aspectRatio: '1:1',
      numImages: 4,
...
export async function processWithImageToImageAndStyle(
  ...
  try {
    ...
    // 在生成请求配置中
    const defaultStylePrompt = "Colorful sketch style pet portrait with vibrant colors, hand-drawn lines, artistic shading, and expressive features";
    ...
    const genPayload: any = {
      ...
      width: outputWidth,
      modelId: '2067ae52-33fd-4a82-bb92-c2c55e7d2786',  // AlbedoBase XL
      // 合并用户提供的提示词(OpenAI英文描述)和默认风格提示词
      prompt: prompt 
    ...
    if (useFastMode) {
      // 快速模式参数调整
      genPayload.num_inference_steps = 30;  // 推理步骤数
      genPayload.guidance_scale = 18;       // 设置为18，提供更强的引导
      genPayload.promptMagic = false;       // 保持关闭提示词魔法
      genPayload.init_strength = 0.9;       // 快速模式下设置为0.9
      logger.info('启用快速模式，调整生成参数');
    } else {
      // 高质量模式参数
      genPayload.num_inference_steps = 50;  // 推理步骤数
      genPayload.guidance_scale = 15;       // 设置为15，提供适中的引导
      genPayload.promptMagic = false;       // 修改：关闭提示词魔法
      genPayload.init_strength = 0.88;      // 高质量模式下设置为0.88
      logger.info('使用高质量模式');
    }
...
export async function generateWithMultipleControlNets(
  ...
  try {
    ...
    const finalGenPayload = {
      ...
      promptStrength: 0.86, // 这里将0.9改为0.86
      guidanceScale: 18, // 从8改为18
    };
